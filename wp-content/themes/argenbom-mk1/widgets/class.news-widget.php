...
class news_widget extends WP_Widget {
  ...
  protected $registered = false;
    ...
    $news['title'] = ! empty( $instance['title'] ) ? $instance['title'] : '';
    $news['news'] = ! empty( $instance['news'] ) ? $instance['news'] : '';

		/** This filter is documented in wp-includes/widgets/class-wp-widget-pages.php */
		$news['title'] = apply_filters( 'widget_title', $news['title'], $instance, $this->id_base );

		$news['text']                  = ! empty( $instance['text'] ) ? $instance['text'] : '';
		 ...
		 */
		$news['text'] = apply_filters( 'widget_text', $news['text'], $instance, $this );
		...
		if ( $is_visual_text_widget ) {
			 ...
			 */
			$news['text'] = apply_filters( 'widget_text_content', $news['text'], $instance, $this );
		} else {
			// Now in legacy mode, add paragraphs and line breaks when checkbox is checked.
			if ( ! empty( $instance['filter'] ) ) {
				$news['text'] = wpautop( $news['text'] );
			}
			...
			if ( has_filter( 'widget_text_content', 'do_shortcode' ) && ! $widget_text_do_shortcode_priority ) {
				if ( ! empty( $instance['filter'] ) ) {
					$news['text'] = shortcode_unautop( $news['text'] );
				}
				$news['text'] = do_shortcode( $news['text'] );
			}
		}
		...
		$news['text'] = preg_replace_callback( '#<(video|iframe|object|embed)\s[^>]*>#i', array( $this, 'inject_video_max_width_style' ), $news['text'] );

		// Adds noreferrer and noopener relationships, without duplicating values, to all HTML A elements that have a target.
    $news['text'] = wp_targeted_link_rel( $news['text'] );
    
    set_query_var('news', $news);
    get_template_part('widgets/news', 'widget');    
	}
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'news' ); ?>">Novedades:</label>
			<select name="<?php echo $this->get_field_name( 'news' ); ?>" id="<?php echo $this->get_field_id( 'news' ); ?>" class="widefat">
        <?php 
          $pages = get_pages(array('parent' => 0)); 
          foreach ( $pages as $page ) {
            $option = '<option value="' . $page->ID . '" '. $this->selected($instance['news'], $page->ID) . '>';
